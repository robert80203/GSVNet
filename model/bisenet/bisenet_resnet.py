...
class BiSeNet(nn.Module):
    ...
    def forward(self, data, label=None):
        ...
        if self.is_training and label is not None:
            '''pred_o1 = F.interpolate(self.heads[0](pred_out[0]), scale_factor=4.0, mode='bilinear',align_corners=True)
            pred_o2 = F.interpolate(self.heads[1](pred_out[1]), scale_factor=4.0, mode='bilinear',align_corners=True)
            pred_o3 = F.interpolate(self.heads[-1](pred_out[2]), scale_factor=4.0, mode='bilinear',align_corners=True)
